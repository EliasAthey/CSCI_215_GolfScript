...
GS operates by placing strings (a.k.a. blocks) on a stack as the interpreter runs through the code. There are roughly 40 built in function titles, the majority of which are single characters, however, many of these chracters can perform mutiple functions depending on the blocks they are acting on. A character with a lot on its plate is (*). The asterisk symbol can mutiply two numbers together, execute a block multiple times, as well as repeat, join, and fold arrays. Despite the small amount of built in characters, GS can be used in an endless number of ways.
